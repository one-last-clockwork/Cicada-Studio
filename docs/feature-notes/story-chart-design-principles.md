...
その中心に置くべきものは、単一サイト内のページ同士を線でつなぐフローチャートではなく、物語上の発見、手がかり、媒体、行動、状態変化を扱う **Story Chart** です。
...
Story Chart は、Site の内側だけを見る画面ではありません。
...
Story Chart 上の Page は、`Site + Page` の組として扱います。

これにより、同じ slug や同じ名前のページが複数 Site に存在しても、物語上の位置を取り違えずに扱えます。

## Story Chart のノード

Story Chart では、少なくとも次のノード種別を扱えるようにします。
...
Story Chart の edge は、単なるリンクやページ遷移ではありません。
...
制作者は、プレイヤーがどの行動を要求されているのかを Story Chart 上で確認できる必要があります。

## 検索システムの位置づけ

ページ内検索は重要な機能です。

しかし、それは Story Chart の一部です。
...
- Project 共通の assets、StoryState、Story Chart metadata を持つ。
...
その制約も Story Chart 上で見えるようにします。

## 制作者が確認できるべきこと

Story Chart は、制作者が次の問いに答えるための画面にします。
...
Story Chart は「自然にページを辿れるか」だけを見る画面ではありません。

プレイヤーの探索行動、Site をまたぐ移動、物語の状態変化を一つの地図として扱う画面です。

## 実装方針

既存のフローチャート機能は、将来的に Story Chart へ発展させます。
...
最終的には、複数 Site、ページ、検索、reveal、unlock、外部探索、偽メッセンジャー、公開タイミング、ending を同じ Story Chart 上で扱えるようにします。
