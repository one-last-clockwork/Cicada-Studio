...
Source Zip は、ページ本文、テーマ、Story Chart、assets、settings を分割し、Git で差分を追いやすい形にします。
...
- Story Chart、conditions、search rules、messenger threads を分割する。
- JSON は安定した並び順で出力する。
...
Project 全体の Story Chart は `story/` に置きます。
...
Project backup には、制作中の情報、未公開ページ、答え、メモ、Story Chart、内部 ID、分岐条件が含まれます。
