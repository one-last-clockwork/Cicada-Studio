...
describe('preview policy', () => {
  it('defaults to a no-script, no-same-origin sandbox and removes scripts', () => {
    ...
    const page = { ...project.sites[0].pages[0], bodyHtml: '<p onclick="alert(1)">x</p><script>window.parent.hacked=true</script>' };
  ...
  it('requires project and page opt-in before scripts can run', () => {
    ...
    const page = { ...project.sites[0].pages[0], allowScripts: true, bodyHtml: '<script>window.flag=1</script>' };
